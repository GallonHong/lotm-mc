...
独立的 Minecraft Bedrock 主世界末日生存怪物与刷怪系统。v0.5.0 新增 8 套由 Deadzone 皮肤、服装、头发、伤口和眼睛图层组合而成的感染者外观，并用服务端同步属性为每只感染者固定外观；所有感染者继续使用具备标准头、身体、四肢挂载骨骼的 Deadzone 人形模型。护甲装备改为逐槽尝试，Test Guns 护甲不存在或注册失败时会自动回退到对应区域的原版护甲，避免一个无效物品导致整套防具不显示。
