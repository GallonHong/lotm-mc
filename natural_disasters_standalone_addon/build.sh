...
VERSIONED_OUT="$ROOT/Natural_Disasters_Standalone_Addon_v1.0.1.mcaddon"
rm -f "$BP_OUT" "$RP_OUT" "$ADDON_OUT" "$ROOT/Natural_Disasters_Standalone_Addon_v1.0.0.mcaddon" "$VERSIONED_OUT"
(cd "$BP" && zip -qr "$BP_OUT" .)
(cd "$RP" && zip -qr "$RP_OUT" .)
TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"' EXIT
