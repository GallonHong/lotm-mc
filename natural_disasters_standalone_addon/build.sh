...
VERSIONED_OUT="$ROOT/Natural_Disasters_Standalone_Addon_v1.3.2.mcaddon"
rm -f "$BP_OUT" "$RP_OUT" "$ADDON_OUT" "$ROOT/Natural_Disasters_Standalone_Addon_v1.0.0.mcaddon" "$ROOT/Natural_Disasters_Standalone_Addon_v1.0.1.mcaddon" "$ROOT/Natural_Disasters_Standalone_Addon_v1.1.0.mcaddon" "$ROOT/Natural_Disasters_Standalone_Addon_v1.2.0.mcaddon" "$ROOT/Natural_Disasters_Standalone_Addon_v1.3.0.mcaddon" "$ROOT/Natural_Disasters_Standalone_Addon_v1.3.1.mcaddon" "$VERSIONED_OUT"
(cd "$BP" && zip -qr "$BP_OUT" .)
(cd "$RP" && zip -qr "$RP_OUT" .)
TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"' EXIT
