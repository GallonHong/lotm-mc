...
versioned_addon="${addon_dir}/SAPI_Server_Addon_v2.7.1.mcaddon"
bundle_dir="$(mktemp -d)"
trap 'rm -rf "${bundle_dir}"' EXIT

rm -f "${bp_pack}" "${addon}" "${addon_dir}/SAPI_Server_Addon_v2.6.4.mcaddon" "${addon_dir}/SAPI_Server_Addon_v2.6.5.mcaddon" "${addon_dir}/SAPI_Server_Addon_v2.7.0.mcaddon" "${versioned_addon}"
