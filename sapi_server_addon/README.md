...
独立的 Minecraft Bedrock 服务器基础系统，源码、测试、构建脚本和安装包全部位于本目录，不再与 LOTM Pathways 源码混放。v2.5.8 将“末日摸金都市”入口固定显示，避免自定义维度启动失败或心跳时序导致按钮消失；未安装摸金 Add-on 时点击不会执行传送。副本入口和自定义寄卖名称等既有功能保持不变。
