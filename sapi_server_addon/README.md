...
独立的 Minecraft Bedrock 服务器基础系统，源码、测试、构建脚本和安装包全部位于本目录，不再与 LOTM Pathways 源码混放。v2.9.1 将保险箱的原生实体生命与 2000 点玩法耐久彻底分离：容器实体使用百万原生生命并逐 tick 回满，也能捕获其他 Addon 直接写入 health 的伤害；玩法耐久归零只切换报废状态，实体、27 格库存和免密码入口继续存在。特殊枪械穿透列表暂时为空。未报废的空保险箱仍可由拥有者回收。经济采用单一金币；Epic 池 20 抽保底，Legendary 池 30 抽保底。
