...
独立的 Minecraft Bedrock 服务器基础系统，源码、测试、构建脚本和安装包全部位于本目录，不再与 LOTM Pathways 源码混放。v2.6.2 将摸金入口改为“始终发送请求 + 三秒回执确认”，不再用可能误判的15秒心跳阻止玩家进入；未收到目标包回执时会明确提醒玩家到现有世界的行为包列表中启用 City BP。v2.6.1 的跨包玩家来源修复及既有服务器功能保持不变。
