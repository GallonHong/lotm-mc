...
独立的 Minecraft Bedrock 服务器基础系统，源码、测试、构建脚本和安装包全部位于本目录，不再与 LOTM Pathways 源码混放。v2.9.2 的七日签到金币为 `2000 / 2500 / 3000 / 3500 / 4000 / 5000 / 8000`（每周期共 28000 金币），并自动迁移仍使用旧内置奖励表的存档；管理员自定义过的签到金额保持不变。同时修正个人银行与全球商店图标，并让内置 Apocalypse 安全区即使联动心跳暂时缺失也始终禁止创建领地。保险箱继续使用独立 2000 点玩法耐久和不可死亡容器实体。
