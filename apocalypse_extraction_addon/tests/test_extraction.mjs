...
assert.deepEqual(manifest.header.version, [0, 3, 3]);
assert.deepEqual(resourceManifest.header.version, [0, 3, 3]);
...
assert.equal(manifest.dependencies.some(dep => dep.uuid), false, "resource and cross-addon dependencies must not block behavior-script startup");
...
assert(main.includes("interop:apoc_extraction_ack") && main.includes("Number(message) || Date.now()"), "SAPI extraction acknowledgement missing");
...
console.log("Apocalypse Extraction City v0.3.3 validation passed.");
