# Apocalypse Extraction City v0.6.0

持久化摸金都市测试版。需要 Minecraft Bedrock/BDS 1.21.120+（26.45 可用）及 Beta APIs 实验玩法。v0.6.0 使用世界属性、会话锁和维度哨兵三重确认城市状态；识别到旧城市后只补齐撤离点和物资箱，不再重复部署 25 个城区。随机入口改为道路到达平台，不再依赖自定义维度中不稳定的地表查询。
