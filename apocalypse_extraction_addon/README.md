# Apocalypse Extraction City v0.10.0

持久化摸金都市测试版。需要 Minecraft Bedrock/BDS 1.21.120+（26.45 可用）及 Beta APIs 实验玩法。v0.10.0 按 RandS 九类地标各自保存的真实地面层下沉结构，消除高楼八至十格地基暴露和入口悬空；高级物资箱改为道路固定补给台，不再随机落到屋顶或建筑内部。

## 安装与联动

- 必装：本 Add-on 中的 `Apocalypse Extraction City BP v0.10.0`、`Apocalypse Extraction Dimension Bootstrap v0.1.0` 和 RP。启用主 BP 时会声明 Bootstrap 依赖。
...
- 首次加载 v0.10.0 时会一次性生成 5×5 共 25 个城区；`city_ready:v7` 和金块布局哨兵写入后，每次进入只检查状态，不重复加载城市结构。
...
从旧版更新后第一次进入会自动执行一次 v0.10.0 地标高度与补给台升级，预计需要 3～6 分钟；完成后不再重建。若旧世界没有自动升级，管理员执行一次 `/scriptevent extract:rebuild`。升级期间不要重复点击入口或重复执行重建。
