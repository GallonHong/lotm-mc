...
独立的 Minecraft Bedrock 每日日常、动态事件与多副本 Addon。v0.11.0 增加联盟每日新闻、14 套报道预设、全服突发通报、事件战报、三种都市传说 Boss 调查及主城叛军入侵；管理员可为新闻事件和持久事件节点手动填写地点名称、维度与 X/Y/Z 坐标。继续接入 Apocalypse Mobs、SAPI Server、Test Guns 与可选载具包。
...
所有敌人优先生成 Apocalypse Mobs 实体，不存在时回退原版实体；载具使用正式 Apocalypse Vehicles 中存在的 `ab_ve:motorcycle` / `ab_ve:truck`，未安装载具包时回退任务标记且路线仍可步行完成。后续增加副本只需新增 Structure、相对坐标、阶段数组和奖励 ID，不需要修改副本状态机。
