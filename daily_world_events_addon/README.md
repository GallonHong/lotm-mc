...
独立的 Minecraft Bedrock 每日日常、动态事件与多副本 Addon。v0.16.2 增加独立 Story Addon 所需的直接创建副本与通关回传接口；v0.16.1 为副本目的地增加灵动视效兼容的自定义全亮光柱与悬浮路径粒子，并保留原版粒子回退；v0.16.0 接入 SAPI 四人队伍：队长选副本后，成员在 30 秒内逐一 Ready，全部通过才创建同一个实例并统一传送；附近在线队友可共享击杀日常、动态事件、Boss 与副本贡献，最终副本奖励仍按玩家独立结算。
