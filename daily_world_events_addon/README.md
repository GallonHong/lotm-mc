...
独立的 Minecraft Bedrock 每日日常、动态事件与多副本 Addon。v0.17.0 将实时事件报道更名为“每日突发事件”，并增加每日一期“希望报”：玩家支付 10,000 金币投稿次日刊登，管理员可排入服务器公告；没有稿件时按照昨日采集、击杀、Boss 和副本统计自动生成主报道。v0.16.3 修复可重复物资箱到期后仍显示开启贴图。
...
奖池、钥匙和刷新时间集中在 `scripts/rewards/lootCratePools.js`。World Event、摸金都市与主世界地图均可直接放置这些方块；交互按“玩家 + 箱子坐标”独立去重。刷新倒计时到期后，已加载箱子会在最多约 5 秒内恢复未开启贴图；到期时未加载的箱子会在区块重新加载后复原。
...
每个真实动态事件启动后都会先写入事件档案，再向全服发布标题、地点名称和完整坐标；事件完成、失败或超时后自动发布战报。玩家可在每日委托菜单的“每日突发事件”查看最近 30 条记录，或使用 `/scriptevent daily:news`。

“希望报”与突发事件档案互不混用。玩家在每日委托菜单投稿，支付 10,000 金币后排入次日刊物，每名玩家每天最多一篇；管理员可免费排入服务器公告。若次日没有玩家稿件或管理员公告，编辑部会根据昨日最突出的采集、击杀、Boss 或副本记录自动生成报道。入口为每日委托菜单或 `/scriptevent daily:hope_post`。
...
从 SAPI 社交菜单创建队伍后，只有队长能够发起副本。系统开始前检查成员在线、存活、未处于其他副本且人数不超过模板上限；任意成员拒绝、关闭 Ready 页面或 30 秒超时都会取消本次进入。没有安装 SAPI 时，本包原有单人创建和实例加入菜单仍可独立工作。
