...
assert.deepEqual(manifest.header.version, [0, 10, 8]);
assert.deepEqual(resourceManifest.header.version, [0, 10, 8]);
...
assert.equal(configModule.CONFIG.extractionPoints.length, 20, "twenty fixed extraction points are required");
assert(configModule.CONFIG.extractionPoints.every(point => Math.abs(point.x) <= 304 && Math.abs(point.z) <= 304), "default extraction points must remain attached to the city deck");
assert.equal(configModule.CONFIG.extractionOutpostVersion, 2, "legacy extraction platform migration version missing");
...
for (const marker of ["buildExtractionMarker", "cleanupLegacyExtractionOutposts", "LEGACY_EXTRACTION_POINTS", "minecraft:black_concrete", "minecraft:lime_concrete", "minecraft:beacon", "minecraft:sea_lantern", "verification expected beacon", "emitNearbyExtractionMarkers", "apoc_extract:extraction_marker", "navigationChatTicks", "publishNavigationChat", "附近没有已确认存在的传说或神话物资箱"]) assert(main.includes(marker), `missing extraction marker/chat navigation behavior: ${marker}`);
...
const extractionParticle = json(join(rp, "particles/extraction_marker.particle.json"));
assert.equal(extractionParticle.particle_effect.description.identifier, "apoc_extract:extraction_marker");
...
assert.deepEqual(json(join(bp, "manifest.json")).header.version, [0, 10, 8]);
assert.deepEqual(json(join(rp, "manifest.json")).header.version, [0, 10, 8]);
console.log("Apocalypse Extraction City v0.10.8 validation passed.");
