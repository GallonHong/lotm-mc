...
独立的 Minecraft Bedrock 每日日常、动态事件与多副本 Addon。v0.19.0 会在玩家首次进入时弹出希望报，并加入每日一次“编辑部真假情报”：答对固定奖励 1,000 金币。v0.18.0 增加普通、高级两种自然分布的实体随机箱：仅生成在主世界法制区/非法制区，摸金都市与安全区不会生成；玩家必须在 1 格内左键或右键开启，物资落地后箱体消失。高级实体箱和无品质废墟方块箱均有独立 1% 概率开出随机 Epic 蓝图。

## 可复用物资箱

- `daily:loot_crate_scavenger`：无品质废墟物资箱，30 分钟刷新；每次获得 1–1000 金币和 2–4 组随机物品。金币中位数为 150，金额越高越罕见；物品权重中至少 40% 为 Addon 食品、饮料、医疗用品、子弹、枪械半成品和载具零件；另有独立 1% 概率追加随机 Epic 图纸，并保留极低概率神话补给密钥；
...
- `daily:random_crate_common`：普通随机箱，在主世界法制区与非法制区分布；金币中位数 60，最多掉落 2 类物品，以原版废旧物资为主，也可能获得子弹、基础半成品和蓝色蓝图；
- `daily:random_crate_advanced`：高级随机箱，只在主世界非法制区分布；金币中位数 120，最多掉落 3 类物品，并有独立 1% 概率把其中一个物品位替换为随机 Epic 蓝图。

两种实体箱都不会在 `apoc_extract:city` 摸金都市维度或安全区生成。普通箱采用约每 6 个候选区块一个、高级箱采用非法制区约每 20 个候选区块一个的确定性分布；同一区块最多存在一个实体箱。右键或左键命中都能开启，但玩家与箱体的三维距离必须不超过 1 格。开启后金币直接到账、物资掉落在地面、箱体立即消失；普通/高级区块分别进入 30/60 分钟再生成冷却。
